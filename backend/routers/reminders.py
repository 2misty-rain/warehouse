...
def read_reminders(is_processed: Optional[bool] = None, db: Session = Depends(get_db)):
    return get_reminders(db, is_processed=is_processed)


@router.post("", response_model=ReminderResponse)
def create_reminder_item(reminder: ReminderCreate, db: Session = Depends(get_db)):
    return create_reminder(db=db, reminder=reminder)


@router.put("/{reminder_id}")
def update_reminder_item(reminder_id: int, is_processed: bool, db: Session = Depends(get_db)):
...
def delete_reminder_item(reminder_id: int, db: Session = Depends(get_db)):
